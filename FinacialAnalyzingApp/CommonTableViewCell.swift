...
class CommonTableViewCell: UITableViewCell {
    ...
    @IBOutlet weak var sectionTitle_lb: UILabel!
    ...
    func openArrow(){
        UIView.animate(withDuration: 0.25, animations: {
            self.arrowIndicatorImageView.transform = CGAffineTransform(rotationAngle: (.pi / 180.0)*0.0);
        })
    }
    
    func closeArrow(){
        UIView.animate(withDuration: 0.25, animations: {
            self.arrowIndicatorImageView.transform = CGAffineTransform(rotationAngle: (.pi / 180.0)*180.0);
        })
    }
